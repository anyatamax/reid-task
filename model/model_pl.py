...
def plot_text_features_2d(text_features, target, batch_captions, current_epoch, global_step):
    ...
    euclidean_distance_matrix = torch.cdist(features_tensor.float(), features_tensor.float(), p=2)
    ...
    fig.suptitle(f'Text Features Visualization - Epoch {current_epoch}, Step {global_step}', 
    ...
    for i, (x, y) in enumerate(features_2d):
        ...
        if has_caption:
            ...
            ax1.scatter(x, y, c=[color_map[label]], s=120, alpha=0.9, 
                        edgecolors='black', linewidth=2, marker='o')
            ax1.annotate(f'ID:{label}', (x, y), xytext=(7, 7), textcoords='offset points', 
                        fontsize=9, alpha=1.0, fontweight='bold')
        else:
            # Точки без captions - меньше, прозрачнее, серая обводка
            ax1.scatter(x, y, c=[color_map[label]], s=60, alpha=0.4, 
                        edgecolors='gray', linewidth=0.5, marker='o')
            ax1.annotate(f'ID:{label}', (x, y), xytext=(3, 3), textcoords='offset points', 
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', 
                ...
                label=f'Without Captions ({len(features_np) - len(valid_caption_indices)})', alpha=0.6)
    ]
    ...
    cosine_distances_flat = cosine_distance_matrix[torch.triu(torch.ones_like(cosine_distance_matrix, dtype=bool), diagonal=1)]
    euclidean_distances_flat = euclidean_distance_matrix[torch.triu(torch.ones_like(euclidean_distance_matrix, dtype=bool), diagonal=1)]
    
    ax4.hist(cosine_distances_flat.numpy(), bins=20, alpha=0.7, label='Cosine Distance', color='blue')
    ax4.hist(euclidean_distances_flat.numpy(), bins=20, alpha=0.7, label='Euclidean Distance', color='red')
    ...
    try:
        ...
        img_buffer.seek(0)
        mlflow.log_artifact(img_buffer, f"text_features_viz/epoch_{current_epoch}_step_{global_step}_2d_viz.png")
...
class CLIPReIDModuleStage1(pl.LightningModule):
    ...
    def on_train_start(self):
        ...
        self.use_graph_sampling = (
            self.trainer.datamodule.use_graph_sampling and
            self.trainer.datamodule._model_for_sampling is not None
        )
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(dataloader):
                ...
                with amp.autocast(self.device.type, enabled=True):
                    ...
                    for i, (target_id, img_feat) in enumerate(zip(target, image_feature)):
            ...
            self.captions_list = captions_list if len([caption for caption in captions_list if caption is not None]) > 0 else None
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(self.device.type, enabled=True):
            text_features = self.model(label=target, get_text=True, captions=batch_captions)
        ...
        if self.cfg.training.solver.text_features_viz and self.global_step % self.cfg.training.solver.text_features_viz_frequency == 0:
            plot_text_features_2d(text_features, target, batch_captions, self.current_epoch, self.global_step)
    ...
    def on_train_epoch_start(self):
        ...
        if self.use_graph_sampling:
            ...
            if self.current_epoch > 1:
                ...
                start_time = time.time()
                print(f"🔄 Epoch {self.current_epoch}: Extracting fresh image features with new graph order...")
                self.extract_image_features()
                elapsed_time = time.time() - start_time
...
class CLIPReIDModuleStage2(pl.LightningModule):
    ...
    def extract_text_features(self):
        ...
        with torch.no_grad():
            ...
            for i in range(i_ter):
                ...
                batch_captions = batch_captions if len([caption for caption in batch_captions if caption is not None]) > 0 else None
                
                with amp.autocast(self.device.type, enabled=True):
                    text_feature = self.model(
                        label=l_list.to(self.device), get_text=True, captions=batch_captions
                    )
                text_features.append(text_feature.cpu())

            self.text_features = torch.cat(text_features, 0).to(self.device)
            print("Captions found for classes in train Stage 2: ", cnt_captions_in_train)
    ...
    def on_train_epoch_start(self):
        ...
        if self.current_epoch > 1:
            print(f"🔄 Stage 2 - Epoch {self.current_epoch}: Re-selecting random captions...")
            self.extract_text_features()
