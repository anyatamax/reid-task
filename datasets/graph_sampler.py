...
class TextGraphSampler(Sampler):
    ...
    def __init__(self, data_source, model, captions_map, batch_size, 
                 num_instance, verbose=False):
        ...
        if self.verbose:
            print(f"TextGraphSampler initialized: {self.num_pids} classes, "
    ...
    def _extract_text_features_for_classes(self):
        ...
        if len(class_text_features) > 0:
            ...
            print(f"TextGraphSampler: len(class_text_features): {len(class_text_features)}")
        else:
    ...
    def _build_similarity_graph(self, features):
        ...
        features_tensor = torch.stack(features) if isinstance(features, list) else features
        ...
        if self.verbose:
            ...
            for i in range(min(3, similarities.shape[0])):
                ...
                print(f"  Class {i}: distances to top neighbors: {top_distances.numpy()[:3]}")
    ...
    def __iter__(self):
        if self.sam_index is None:
            print("⚠️  Warning: Graph sampling index not built, calling make_index()...")
            self.make_index()
