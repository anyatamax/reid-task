...
Использовался алгоритм Graph Sampling https://arxiv.org/pdf/2104.01546 на первой
стадии для тектовых фичей.
...
Так же пробовала первые 30 эпох рандомно выбирать батч, а остальные 70 через
Graph Sampling. Причем пересчитывать не каждую эпоху, а каждую пятую эпоху
...
Использовались текстовые описания из датасета дял тех изображений для которых
нашлись, они не тренировались на первой стадии, потому что у таких описаний не
было обучаемых токенов. (эмбеддинги не правильно тренить, потому что они тогда
заафектят тексы для которых нет caption, как вариант потом добавить к них
обучаемые токены)

Captions in train Stage 1: 3078 Number of images in train Stage 1: 12936 ->
нашлось для 23% изображений
